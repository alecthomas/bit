...
## How does Bit improve on Make?

### Native support for globs, including against outputs

Inputs for a target can be globs. Globs are matched against the filesystem, as
well as declared outputs of other targets.

eg. Given the following build file, `build/tool` will be rebuilt if any `.go`
file changes. In particular, even if the generated file `token_enumer.go` is
deleted it will still be included as a dependency.
...
### Builtin (but overridable) cleaning

`bit --clean` can be used to remove all declared outputs of all targets. This
behaviour can be overridden per target, including disabling it.
...
Bit verifies that the declared outputs of a target are generated by that target
during the build, and will raise an error if any are missing.
